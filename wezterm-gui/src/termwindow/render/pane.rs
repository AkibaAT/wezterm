    ...
    pub fn paint_pane(
        ...
        */
        ...
        if self.show_scroll_bar {
            ...
            // Semi-transparent track behind the thumb
            let track_color = LinearRgba::with_components(0.5, 0.5, 0.5, 0.15);
            let mut track_quad = self
                .filled_rectangle(
                    layers,
                    1,
                    euclid::rect(
                ...
                .context("scrollbar track filled_rectangle")?;
            track_quad.set_hsv(if pos.is_active {
                None
            } else {
                Some(config.inactive_pane_hsb)
            });
